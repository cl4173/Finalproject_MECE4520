...
N_trails=9   #Define loops(random seed)
...
#package function
def Regression(n_trail:int,order:int,p_c:float,l1_ratio_step:float,Itol:float,r_type:int,r_solver:str,m_iter:int,l1_ratio:float,data):
    ...
    for i in range(n_trail):
        ...
        X_tr, X_te, y_train, y_test = train_test_split(I_data_x, I_data_y, test_size=0.2, random_state=(19))  
...
#main calc
H_Parameter_Kfold,H_Parameter_Trail=Regression(n_trail=N_trails,order=Order,p_c=P_C,l1_ratio_step=L1_Ratio_step,Itol=Tol,r_type=R_type_A,
